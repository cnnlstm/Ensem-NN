...
def matrix_quaternion(R):
  """Return quaternion from homogeneous rotation matrix.
  """  
  trace = np.trace(R)
  if trace > 0:
...
def draw_skeleton(joints_list,show=0,save=0,autoscale=0,outdir="",outname="",scale=1):
  #plt.ion()
  fig = plt.figure()
...
def normalize_skeleton(jinfo,anchor=None,norm_dist=0,right_to_left=None,spine_to_top=None):
  ...
  rotated_and_norm = {}
  for jnum, joint in norm_joints.items():
  ...
  rotated_and_norm2 = {}
  for jnum, joint in rotated_and_norm.items():
...
def generate_data(argv):
  ...
  g = open("test_list.txt","a")
  w = 0
  bad_files = vids_with_missing_skeletons()
  skeleton_dir_root = "/home/siat/xyy/dataset/nturgbd/nturgb+d_skeletons/"
  skeleton_files = os.listdir(skeleton_dir_root)
  data_out_dir = '/home/siat/xyy/dataset/nturgbd/hdf5_file_differ/'
  ...
  for file_name in skeleton_files:
    ...
    feature = np.zeros((300, feat_dim))   #[帧数,150]
    for n in range(0,num_frames):
      ...
      if body_count > 2:
          ...
          aasa.append(file_name)
          #print len(set(aasa)),"files have 3 bodies" 
        ...
        for b in range(0,body_count):
          ...
          for j in range(0,joint_count):
            ...
            jinfo[j] = (x,y,z,rw,rx,ry,rz) #jinfo = [25:7]
          ## END JOINT LOOP
          ...
          binfo[b] = jinfo  #body_nums = [body_count:25*7]
      
        ## END BODY LOOP
        sample_ind = 0
        sample = np.zeros(feat_dim)    #25*3 = 75,每个body有75个点就能确定，最多有两个人，因此feat_dim = 150。一个人的用0补齐。
        
        ## CONSTRUCT THE FEATURE FOR THIS N-th FRAME
        for bind, body in binfo.items():     #每一个身体
          for jind, joint in body.items():    #joint = [7]
            ...
            sample_ind += 1
        feature[n] = sample                #feature: [n帧：150]
      ## END BODY COUNT IF-ELSE
    ## END FRAME LOOP

    if body_count <= 2:
      feature_differ = np.zeros((300, feat_dim))
      for row in range(feature.shape[0]):
        if row > 0:
          feature_differ[row] = feature[row] - feature[(row-1)]
      arr_path = data_out_dir+file_name.split(".")[0]+'.h5'
      f = h5py.File(arr_path,'w')   
      f['data'] = feature_differ
      f["label"] = one_hot
      f.close() 
      #print feature_differ.shape
      k =k+1
      print k
...
'''
  ...
  print("Writing out data . . . ")          #X_train：[videos_num,n帧,150]
  
  train_max_len = max([len(c) for c  in X_train])   #max_len代表最长的帧数目
  ...
  for i in range(0,len(X_train)):
    ...
    X = np.zeros((max_len,feat_dim))   # [300,150]
    num_rows = X_train[i].shape[0]     # n帧
    X[0:num_rows] =  X_train[i]        # [videos_num,n帧,150]
